...
; rgbasm -o color_bands_with_moving_window.obj color_bands_with_moving_window.asm
; rgblink -o color_bands_with_moving_window.gb color_bands_with_moving_window.obj
; rgbfix -v -p 0 color_bands_with_moving_window.gb
; ------------------------------------------------
; ------------------------------------------------
;   color_bands_with_moving_window.asm ---
...
.enableLCD
    ; Initialize window X position variable
    ld a, 87
    ld [wWindowX], a    ; Store initial WX value in RAM

    ; Enable VBlank interrupt
    ld a, %00000001     ; Bit 0: VBlank interrupt enable
    ldh [$ffff], a      ; rIE (Interrupt Enable) <= a
...
; VBlank interrupt handler - moves window right by 2 pixels
VBlankHandler:
    ...
    ; Load current window X position
    ld a, [wWindowX]
    add a, 2            ; Move right by 2 pixels

    ; Check if window has moved too far right (wrap around)
    cp 167              ; If WX >= 167, reset to start position
    jr c, .updateWX
    ld a, 87            ; Reset to initial position (appears at X=80)

.updateWX:
    ld [wWindowX], a    ; Store new position in RAM
    ;;;;;;;; BREAKPOINT 2 ;;;;;;;;;;;;;
    ld hl, $FFFE   ; HL = target address
    ld e, $AC
    ld [hl], e      ; keep an eye on the value of b (ROW). Make sure it goes to 32
