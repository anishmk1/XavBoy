...
; rgbasm -o simple_objects.obj simple_objects.asm
; rgblink -o simple_objects.gb simple_objects.obj
; rgbfix -v -p 0 simple_objects.gb
; ------------------------------------------------
; ------------------------------------------------
;   simple_objects.asm ---
;       Draws a color band background and a couple objects on screen
...
Start:
    ...
    ; TILE 4 ($8040) -- WINDOW OBJECT TILE
    ld hl, TileWindow       ; HL = source in ROM
...
;           Draw Object Tiles         ;
;-------------------------------------;
; Construct object using the object data format
; Write it to OAM
...
DrawObjects:
    ...
    ld [hl+], a
    ld a, 4             ; Byte 2 = Tile Index (Point to Window tile)
    ld [hl+], a
    ld a, %10000000     ; Byte 3 - Attributes/Flags - same for all objects
    ld [hl+], a

    ; place one at 0, 100
    ld a, 116            ; Byte 0 = Y position 
    ld [hl+], a
    ld a, 8             ; Byte 1 = X position 
    ld [hl+], a
    ld a, 4             ; Byte 2 = Tile Index (Point to Window tile)
    ld [hl+], a
    ld a, %10000000     ; Byte 3 - Attributes/Flags - same for all objects
    ...
    ; place one at 100, 0
    ld a, 16            ; Byte 0 = Y position 
    ld [hl+], a
    ld a, 108           ; Byte 1 = X position 
    ld [hl+], a
    ld a, 4             ; Byte 2 = Tile Index (Point to Window tile)
    ld [hl+], a
    ld a, %10000000     ; Byte 3 - Attributes/Flags - same for all objects
